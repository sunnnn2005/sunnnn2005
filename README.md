# Hi, I'm Shun Lu

UC Davis Data Science student building backend systems, data tools, AI-agent workflows, and iPad-native note-taking products.

I am currently looking for SWE, data, ML, and AI engineering internship opportunities.

## Featured Projects

### SignalRoot Agent

Agentic incident-triage platform that correlates alerts, metrics, logs, deploy history, and known incident signatures to generate root-cause reports.

- FastAPI backend
- Interactive dashboard
- Weighted evidence ranking
- Agent trace output
- Docker and GitHub Actions CI

Repo: https://github.com/sunnnn2005/signalroot-agent

### Data Quality Agent

Agentic data reliability platform that profiles datasets and detects schema drift, missing values, duplicate keys, freshness failures, volume anomalies, and numeric outliers.

- FastAPI backend
- pandas-based profiling and checks
- Severity-weighted quality score
- Likely-cause and remediation generation
- Docker and GitHub Actions CI

Repo: https://github.com/sunnnn2005/data-quality-agent

### Yizhi Xinyi

Native iPad note-taking app that turns handwritten headings into a navigable table of contents for student lecture notes.

- SwiftUI
- PencilKit
- Vision OCR
- Multi-page handwritten notes
- Folder-based organization

Repo: https://github.com/sunnnn2005/smart-handwritten-note-taking-app

## Tech Stack

- Languages: Python, SQL, R, JavaScript, Swift
- Data: pandas, NumPy, matplotlib, seaborn, regression analysis
- Backend: FastAPI, Node.js, Express, REST APIs, PostgreSQL
- Tools: Docker, GitHub Actions, Git, GitHub
- iPadOS: SwiftUI, PencilKit, Vision OCR

## Links

- LinkedIn: https://www.linkedin.com/in/shun-lu-20060b210
- Email: shunlu2005@gmail.com
